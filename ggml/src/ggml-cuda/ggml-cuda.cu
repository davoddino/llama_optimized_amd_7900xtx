...
static ggml_cuda_device_info ggml_cuda_init() {
    ...
    if (ggml_cuda_rdna3_qwen36_superlayer_env_present()) {
        const bool qwen36_final_env = ggml_cuda_rdna3_qwen36_superlayer_final_requested();
        const bool qwen36_mega_decode_effective =
            ggml_cuda_env_enabled("GGML_CUDA_RDNA3_QWEN36_MEGA_DECODE") || qwen36_final_env;
        const bool qwen36_mega_required_effective =
        ...
        const bool qwen36_one_layer_mega_unsafe =
            ggml_cuda_env_enabled("GGML_CUDA_RDNA3_QWEN36_ONE_LAYER_MEGA_UNSAFE") ||
            ggml_cuda_env_enabled("GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_FINAL_PHYSICAL_L0_UNSAFE");
        const bool qwen36_one_layer_mega_final_auto =
            qwen36_final_env &&
            !ggml_cuda_rdna3_qwen36_superlayer_final_physical_l0_requested();
        const bool qwen36_one_layer_mega_effective =
            qwen36_one_layer_mega_final_auto ||
            (qwen36_one_layer_mega_unsafe &&
             (ggml_cuda_env_enabled("GGML_CUDA_RDNA3_QWEN36_ONE_LAYER_MEGA") ||
              ggml_cuda_rdna3_qwen36_superlayer_final_physical_l0_requested()));
        GGML_LOG_INFO(
                ...
                qwen36_mega_decode_effective ? 1 : 0,
                ...
                qwen36_one_layer_mega_final_auto ? 1 : 0,
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    static const bool log_mmid_path = getenv("GGML_CUDA_RDNA3_PROFILE_LOG") != nullptr;

    auto log_mul_mat_id_path = [&](const char * path) {
        if (log_mmid_path) {
            GGML_LOG_INFO("%s: MUL_MAT_ID path=%s: dst=%s, src0_type=%s, experts=%" PRId64
                    ", expert_used=%" PRId64 ", tokens=%" PRId64 ", out=%" PRId64 "\n",
                    __func__, path, dst->name, ggml_type_name(src0->type), ne02, ids->ne[0], ne12, ne0);
        }
    };
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        if (ne2 <= MMVQ_MAX_BATCH_SIZE) {
            if (ggml_is_quantized(src0->type)) {
                ...
                if (ne2 <= mmvq_mmid_max) {
                    log_mul_mat_id_path("MMVQ");
                    ggml_cuda_mul_mat_vec_q(ctx, src0, src1, ids, dst);
                    return;
                }
            } else {
                if (GGML_CUDA_CC_IS_AMD(cc)) {
                    log_mul_mat_id_path("MMVF_AMD");
        ...
        if (ggml_cuda_should_use_mmq(src0->type, cc, ne12, /*n_experts=*/ne02)) {
            log_mul_mat_id_path("MMQ");
            ggml_cuda_mul_mat_q(ctx, src0, src1, ids, dst);
            return;
        }

        if (ggml_cuda_should_use_mmf(src0->type, cc, WARP_SIZE, src0->ne, src0->nb, src1->ne[2], /*mul_mat_id=*/true)) {
            log_mul_mat_id_path("MMF");
    ...
    static const bool log_mmid_host_sync =
        getenv("GGML_CUDA_RDNA3_PROFILE_LOG") != nullptr ||
        getenv("GGML_CUDA_MUL_MAT_ID_FALLBACK_LOG") != nullptr;
...
static ggml_cuda_rdna3_qwen36_one_layer_mega_plan ggml_cuda_rdna3_qwen36_one_layer_mega_make_plan(
    ...
    plan.conv_raw       = find_named_node("conv_output_raw");
    plan.conv_silu      = find_named_node("conv_output_silu");
    ...
    const bool projection_bundle_nodes_present =
    ...
    if (projection_bundle_nodes_present) {
...
static bool ggml_cuda_rdna3_qwen36_one_layer_mega_launch(
    ...
    bool projection_bundle_ok =
        plan.projection_bundle &&
        ggml_cuda_rdna3_qwen36_one_layer_proj_bundle_enabled();
...
static bool ggml_cuda_set_rows_pair_type_supported(const ggml_type type) {
    switch (type) {
        case GGML_TYPE_F32:
        case GGML_TYPE_F16:
        case GGML_TYPE_BF16:
...
static bool ggml_cuda_tensor_logical_ranges_overlap(const ggml_tensor * a, const ggml_tensor * b) {
...
static bool ggml_cuda_should_fuse_set_rows_pair(const ggml_tensor * set_rows0, const ggml_tensor * set_rows1) {
    ...
    const ggml_tensor * src00 = set_rows0->src[0];
    const ggml_tensor * src01 = set_rows1->src[0];
    const ggml_tensor * idx0  = set_rows0->src[1];
...
struct ggml_cuda_moe_weighted_sum_fusion_info {
...
static bool ggml_cuda_moe_weighted_sum_fusion(
        const ggml_cgraph * cgraph,
        const int          node_idx,
        ggml_cuda_moe_weighted_sum_fusion_info & info) {
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    if (!GGML_CUDA_CC_IS_RDNA3(cc) || getenv("GGML_CUDA_RDNA3_DISABLE_MOE_COMBINE") != nullptr) {
        return false;
    }

    if (node_idx >= cgraph->n_nodes || cgraph->nodes[node_idx]->op != GGML_OP_MUL) {
        return false;
    }

    ggml_tensor * weighted = cgraph->nodes[node_idx];
    if (weighted->type != GGML_TYPE_F32) {
        return false;
    }
    ...
    if (experts->ne[0] == 1 && weights->ne[0] > 1) {
        std::swap(experts, weights);
    }
    ...
    if (n_embd <= 0 || n_tokens <= 0 || n_expert_used < 2 || n_expert_used > 8) {
        return false;
    }

    if (weights->ne[0] != 1 || weights->ne[1] != n_expert_used || weights->ne[2] != n_tokens) {
        return false;
    }
    if (weighted->ne[0] != n_embd || weighted->ne[1] != n_expert_used || weighted->ne[2] != n_tokens) {
        return false;
    }

    const int n_expert_used_i = int(n_expert_used);
    const int views_start = node_idx + 1;
    const int adds_start  = views_start + n_expert_used_i;
    const int last_node   = adds_start + n_expert_used_i - 2;
    if (last_node >= cgraph->n_nodes) {
        return false;
    }

    for (int i = 0; i < n_expert_used_i; ++i) {
    ...
    for (int i = 1; i < n_expert_used_i; ++i) {
    ...
    for (int i = 0; i < n_expert_used_i - 2; ++i) {
    ...
    info.dst     = cgraph->nodes[last_node];
    info.n_nodes = 2*n_expert_used_i;
...
static bool ggml_cuda_can_fuse(const struct ggml_cgraph *                cgraph,
    ...
    std::initializer_list<enum ggml_op> mul_mat_scale_glu_ops    = { GGML_OP_MUL_MAT,    GGML_OP_MUL, GGML_OP_MUL_MAT,    GGML_OP_MUL, GGML_OP_GLU };
    std::initializer_list<enum ggml_op> mul_mat_id_scale_glu_ops = { GGML_OP_MUL_MAT_ID, GGML_OP_MUL, GGML_OP_MUL_MAT_ID, GGML_OP_MUL, GGML_OP_GLU };
    ...
    if ((is_equal(mul_mat_scale_glu_ops, ops) || is_equal(mul_mat_id_scale_glu_ops, ops)) &&
        ggml_can_fuse_subgraph(cgraph, node_idx, ops, { node_idx + 4 })) {
        const ggml_tensor * ffn_a   = cgraph->nodes[node_idx];
        const ggml_tensor * scale_a = cgraph->nodes[node_idx + 1];
        const ggml_tensor * ffn_b   = cgraph->nodes[node_idx + 2];
        const ggml_tensor * scale_b = cgraph->nodes[node_idx + 3];
        const ggml_tensor * glu     = cgraph->nodes[node_idx + 4];

        const ggml_tensor * gate_scale = glu->src[0];
        const ggml_tensor * up_scale   = glu->src[1];
        const ggml_tensor * ffn_gate   = nullptr;
        const ggml_tensor * ffn_up     = nullptr;

        if (gate_scale == scale_a && up_scale == scale_b) {
            ffn_gate = ffn_a;
            ffn_up   = ffn_b;
        } else if (gate_scale == scale_b && up_scale == scale_a) {
            ffn_gate = ffn_b;
            ffn_up   = ffn_a;
        } else {
            return false;
        ...
        auto get_mul_scale_tensor = [](const ggml_tensor * scale_node, const ggml_tensor * mul_node) -> const ggml_tensor * {
            if (scale_node->src[0] == mul_node) {
                return scale_node->src[1];
            }
            if (scale_node->src[1] == mul_node) {
                return scale_node->src[0];
            }
            return nullptr;
        };

        const ggml_tensor * up_scale_tensor   = get_mul_scale_tensor(up_scale, ffn_up);
        const ggml_tensor * gate_scale_tensor = get_mul_scale_tensor(gate_scale, ffn_gate);

        if (up_scale_tensor == nullptr || gate_scale_tensor == nullptr) {
            return false;
        }

        const bool up_scale_scalar   = ggml_nelements(up_scale_tensor) == 1;
        const bool gate_scale_scalar = ggml_nelements(gate_scale_tensor) == 1;

        if (up_scale_tensor->type != GGML_TYPE_F32 || gate_scale_tensor->type != GGML_TYPE_F32 ||
            (!up_scale_scalar && up_scale_tensor->ne[0] != glu->ne[0]) ||
            (!gate_scale_scalar && gate_scale_tensor->ne[0] != glu->ne[0])) {
            return false;
        }
    ...
    auto can_fuse_rms_norm_silu_mul = [&](const int node_count,
                                          const ggml_tensor * rms_norm,
                                          const ggml_tensor * weight_mul,
                                          const ggml_tensor * silu,
                                          const ggml_tensor * out_mul) {
        ...
        int out_nodes[] = { node_idx + node_count - 1 };
        return ggml_cuda_check_fusion_memory_ranges(cgraph, node_idx, node_count, out_nodes, 1);
    };
    ...
    if (is_equal(rms_norm_silu_mul_ops, ops) &&
        ...
        return can_fuse_rms_norm_silu_mul(4, rms_norm, weight_mul, silu, out_mul);
    }

    std::initializer_list<enum ggml_op> silu_rms_norm_mul_ops = { GGML_OP_UNARY, GGML_OP_RMS_NORM, GGML_OP_MUL, GGML_OP_MUL };

    if (is_equal(silu_rms_norm_mul_ops, ops) &&
        unary_ops.size() == 1 && unary_ops.begin()[0] == GGML_UNARY_OP_SILU &&
        ggml_can_fuse_subgraph(cgraph, node_idx, ops, { node_idx + 3 })) {
        const ggml_tensor * silu       = cgraph->nodes[node_idx];
        const ggml_tensor * rms_norm   = cgraph->nodes[node_idx + 1];
        const ggml_tensor * weight_mul = cgraph->nodes[node_idx + 2];
        const ggml_tensor * out_mul    = cgraph->nodes[node_idx + 3];
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            std::vector<bool> qwen36_one_layer_projection_bundle_launched(
                    one_layer_plans != nullptr ? one_layer_plans->size() : 0, false);
            const bool qwen36_one_layer_mega_launch_required =
            ...
            const bool qwen36_superlayer_l0_replace_proj_z_math_only =
            ...
            const bool qwen36_superlayer_l0_replace_ssm =
                superlayer_l0_plan != nullptr &&
                ggml_cuda_rdna3_qwen36_superlayer_replace_l0_ssm_enabled(cuda_ctx->device);
            const bool qwen36_superlayer_l0_replace_l2 =
                superlayer_l0_plan != nullptr &&
                ggml_cuda_rdna3_qwen36_superlayer_replace_l0_l2_enabled(cuda_ctx->device);
            ...
            const uint32_t qwen36_superlayer_l0_ssm_mask =
                qwen36_superlayer_l0_replace_ssm ? 0x40u : 0u;
            const uint32_t qwen36_superlayer_l0_l2_mask =
            ...
            for (int i = 0; i < cgraph->n_nodes; i++) {
                ...
                if (superlayer_l0_plan != nullptr &&
                        !qwen36_superlayer_l0_ssm_launched &&
                        i == superlayer_l0_plan->conv_raw &&
                        qwen36_superlayer_l0_ssm_mask != 0) {
                    qwen36_superlayer_l0_launch_mask(i, "ssm", qwen36_superlayer_l0_ssm_mask);
                    qwen36_superlayer_l0_ssm_launched = true;
                }
                if (superlayer_l0_plan != nullptr &&
                        !qwen36_superlayer_l0_l2_launched &&
                        i == superlayer_l0_plan->q_l2 &&
                        qwen36_superlayer_l0_l2_mask != 0) {
                    qwen36_superlayer_l0_launch_mask(i, "l2", qwen36_superlayer_l0_l2_mask);
                    qwen36_superlayer_l0_l2_launched = true;
                }
                if (superlayer_l0_plan != nullptr &&
                        !qwen36_superlayer_l0_gdn_launched &&
                        i == superlayer_l0_plan->gdn &&
                        qwen36_superlayer_l0_gdn_mask != 0) {
                ...
                const bool qwen36_superlayer_l0_skip =
                    ...
                    ((qwen36_superlayer_l0_replace_rms &&
                     ...
                     (qwen36_superlayer_l0_replace_ssm &&
                      (i == superlayer_l0_plan->conv_raw ||
                       i == superlayer_l0_plan->conv_silu)) ||
                     (qwen36_superlayer_l0_replace_l2 &&
                      (i == superlayer_l0_plan->q_l2 ||
                ...
                if (qwen36_one_layer_qkv_hit) {
                    ...
                    const bool qkv_ok = ggml_cuda_rdna3_qwen36_one_layer_mega_launch(
                            ...
                            &projection_bundle_launched);
                    if (!qkv_ok) {
                        if (qwen36_one_layer_mega_launch_required) {
                            GGML_ABORT("%s: Qwen3.6 RDNA3 one-layer QKV mega failed: %s",
                                    __func__, qkv_blocker.c_str());
                        }
                        static std::atomic<int64_t> qwen36_one_layer_fallback_report_count{0};
                        const int64_t fallback_report_id =
                            qwen36_one_layer_fallback_report_count.fetch_add(1, std::memory_order_relaxed);
                        if (fallback_report_id < 16 || ggml_cuda_rdna3_graph_log_enabled(cuda_ctx->device)) {
                            GGML_LOG_INFO(
                                    "rdna3_qwen36_one_layer_mega: fallback-original layer=%d"
                                    " node=%d reason=%s\n",
                                    plan.layer, i, qkv_blocker.c_str());
                        }
                    } else {
                ...
                if (!disable_fusion) {
                    ...
                    if (can_fuse_linear_stream_region(2) &&
                            ggml_can_fuse_subgraph(cgraph, i, { GGML_OP_SET_ROWS, GGML_OP_SET_ROWS }, { i, i + 1 }) &&
                            ggml_cuda_should_fuse_set_rows_pair(node, cgraph->nodes[i + 1])) {
                        op_timer.set_path("SET_ROWS_PAIR_FUSED");
                        ggml_cuda_op_set_rows_pair(*cuda_ctx, node, cgraph->nodes[i + 1]);
                        i++;
                        continue;
                    }
                    ...
                    if (node->op == GGML_OP_MUL) {
                        ggml_cuda_moe_weighted_sum_fusion_info moe_sum;
                        if (ggml_cuda_moe_weighted_sum_fusion(cgraph, i, moe_sum)) {
                            if (getenv("GGML_CUDA_RDNA3_PROFILE_LOG") != nullptr) {
                                GGML_LOG_INFO("%s: MOE_WEIGHTED_SUM_FUSED n_embd=%" PRId64
                    ...
                    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
                        ...
                        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
                        ...
                        } else if (ggml_cuda_can_fuse(cgraph, i, { op, GGML_OP_MUL, op, GGML_OP_MUL, GGML_OP_GLU }, {})) {
                            ggml_tensor * glu          = cgraph->nodes[i + 4];
                            ggml_tensor * gate_scale_n = glu->src[0];
                            ggml_tensor * up_scale_n   = glu->src[1];

                            ggml_tensor * gate_n = nullptr;
                            ggml_tensor * up_n   = nullptr;

                            if (gate_scale_n == cgraph->nodes[i + 1] && up_scale_n == cgraph->nodes[i + 3]) {
                                gate_n = cgraph->nodes[i];
                                up_n   = cgraph->nodes[i + 2];
                            } else if (gate_scale_n == cgraph->nodes[i + 3] && up_scale_n == cgraph->nodes[i + 1]) {
                                gate_n = cgraph->nodes[i + 2];
                                up_n   = cgraph->nodes[i];
                            } else {
                                continue;
                            ...
                            auto get_scale_tensor = [](const ggml_tensor * scale_node, const ggml_tensor * mul_node) {
                                if (scale_node->src[0] == mul_node) {
                                    return scale_node->src[1];
                                }
                                if (scale_node->src[1] == mul_node) {
                            ...
                            ggml_tensor * up_scale_tensor   = get_scale_tensor(up_scale_n, up_n);
                            ggml_tensor * gate_scale_tensor = get_scale_tensor(gate_scale_n, gate_n);

                            if (!up_scale_tensor || !gate_scale_tensor) {
                                continue;
                            }
                            ...
                            if (ggml_cuda_should_fuse_mul_mat_vec_f(up_n)) {
                                ggml_cuda_mm_fusion_args_host fusion_data{};
                                fusion_data.gate       = gate_n->src[0];
                                fusion_data.x_scale    = up_scale_tensor;
                                fusion_data.gate_scale = gate_scale_tensor;
                                fusion_data.glu_op     = ggml_get_glu_op(glu);

                                op_timer.set_path("MUL_MAT_VEC_F_GLU_SCALE_FUSED");
                                ggml_cuda_mul_mat_vec_f(*cuda_ctx, src0, src1, ids, glu, &fusion_data);
                                fused_mul_mat_vec = true;
                                fused_node_count = 5;
                                break;
                            ...
                            if (ggml_cuda_should_fuse_mul_mat_vec_q(up_n)) {
                                ...
                                op_timer.set_path("MUL_MAT_VEC_Q_GLU_SCALE_FUSED");
                                ggml_cuda_mul_mat_vec_q(*cuda_ctx, src0, src1, ids, glu, &fusion_data);
                                fused_mul_mat_vec = true;
                                fused_node_count = 5;
                                break;
                    ...
                    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
                        if (!ggml_can_fuse(cgraph, i, { op, GGML_OP_MUL })) {
                            continue;
                        }
                        ...
                        if (scale_node->src[0] == mm_node) {
                            scale_tensor = scale_node->src[1];
                        } else if (scale_node->src[1] == mm_node) {
                            scale_tensor = scale_node->src[0];
                        } else {
                            continue;
                        }

                        const bool scale_scalar = ggml_nelements(scale_tensor) == 1;
                        if (scale_tensor->type != GGML_TYPE_F32 ||
                                (!scale_scalar && scale_tensor->ne[0] != scale_node->ne[0])) {
                            continue;
                        }
                        ...
                        if (ggml_cuda_should_fuse_mul_mat_vec_f(mm_node)) {
                            op_timer.set_path("MUL_MAT_VEC_F_SCALE_FUSED");
                            ggml_cuda_mul_mat_vec_f(*cuda_ctx, src0, src1, ids, scale_node, &fusion_data);
                            fused_mul_mat_vec = true;
                            fused_node_count = 2;
                            break;
                        }
                        ...
                        if (ggml_cuda_should_fuse_mul_mat_vec_q(mm_node)) {
                            op_timer.set_path("MUL_MAT_VEC_Q_SCALE_FUSED");
                            ggml_cuda_mul_mat_vec_q(*cuda_ctx, src0, src1, ids, scale_node, &fusion_data);
                            fused_mul_mat_vec = true;
                            fused_node_count = 2;
                            break;
                        }
                    ...
                    if (can_fuse_linear_stream_region(4) &&
                        ...
                        op_timer.set_path("RMS_NORM_SILU_MUL_FUSED");
                        ggml_cuda_op_rms_norm_fused_silu_mul(*cuda_ctx, cgraph->nodes[i+1], cgraph->nodes[i+2], node, cgraph->nodes[i+3]);
                        i += 3;
                        continue;
                    }
...
static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    if (qwen36_mega_decode) {
        ...
        if (!qwen36_superlayer_final &&
                (rdna3_graph_log || !mega_ok || report_id < 4 || report_uid_change)) {
        ...
        if (qwen36_one_layer_mega && qwen36_mega_contract_this_eval) {
            ...
            if (final_auto_layers && ready_layers == 0 && qwen36_one_layer_mega_required) {
