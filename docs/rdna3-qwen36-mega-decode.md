...
On ROCm builds without CUB, full-vocabulary `top_k` and `top_p` backend sampling are not supported because they require large `TOP_K` / `ARGSORT` ops. The `mega-contract` helper therefore switches the default server sampler chain to `min_p;temperature` and disables `top_k` / `top_p` for the server defaults. The final distribution sampler is appended by the common sampler chain.
