...
`GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_SMOKE=1` is kept as an alias for `DISPATCH=1`. `DISPATCH=1` alone no longer materializes or launches per-token work; it only enables the channel. `SUPERLAYER=1` is the explicit enable, but `REQUIRED`, `CONTRACT`, `RUN_L0`, `SINGLE_L0_DISPATCH`, or any `REPLACE_L0*` flag also implies the physical superlayer path. Set `CONTRACT=1` to materialize the runtime pack and launch the scaffold cooperative contract kernel without L0 math. Set `RUN_L0=1` to execute the L0 RMSNorm/QKV/projection math without skipping the normal graph. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_REPLACE_L0` also defaults to `0`; setting it to `1` implies L0 RMS/QKV/projection replacement and skips those graph nodes. The narrower `REPLACE_L0_RMS`, `REPLACE_L0_QKV`, `REPLACE_L0_PROJ`, `REPLACE_L0_SSM`, `REPLACE_L0_L2`, and `REPLACE_L0_GDN` flags replace only that L0 substage and are intended for correctness isolation. The still narrower `REPLACE_L0_PROJ_Z`, `REPLACE_L0_PROJ_BETA`, and `REPLACE_L0_PROJ_ALPHA` flags replace only one projection branch and skip only that branch's graph nodes. `SINGLE_L0_DISPATCH=1` is a full L0 physical preset for the RMS/QKV/projection prefix: it implies RMS, QKV, Z, BETA, and ALPHA replacement and collapses them into one cooperative dispatch at `norm-0` with stage mask `0x1f`.

`GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_FINAL=1` is fail-closed. It must not execute or advertise the current scaffold path as the final megakernel. Until the real numeric 40-layer Qwen3.6 RDNA3 dataflow exists inside the physical superlayer, final mode aborts with an explicit blocker. L0/one-layer physical replacement remains available through the non-final `RUN_L0`, `REPLACE_L0*`, and `SINGLE_L0_DISPATCH` controls for correctness isolation, but it is not the objective-complete megakernel.

Partial projection isolation still uses the latest enabled projection endpoint unless `SINGLE_L0_DISPATCH=1` is set. `REPLACE_L0_PROJ_Z_MATH_ONLY=1` computes and skips only the raw `z` matmul, then lets the graph run the public `z` node for isolation. `REPLACE_L0_SSM=1` requires `GGML_CUDA_RDNA3_SSM_CONV_STATE_INPLACE=1`; it launches at `conv_output_raw-0`, computes the state-token SSM conv, writes `conv_output_raw-0`, writes `conv_output_silu-0`, updates `state_update_target-0`, and skips the original SSM conv plus SILU nodes. `REPLACE_L0_L2=1` launches at `q_conv_predelta-0`, normalizes the Q/K conv views in the cooperative superlayer dispatch, writes `q_conv_predelta-0` and `k_conv_predelta-0`, and skips the original L2 pair nodes. `REPLACE_L0_GDN=1` requires `GGML_CUDA_RDNA3_GDN_STATE_INPLACE=1`; it launches at `__fgdn_ar__-0`, computes the one-token Gated Delta Net output, writes the in-place recurrent state target, and skips the original GDN node. `DIRECT_L0_WEIGHTS=1` reads the implemented L0 RMSNorm, QKV, projection, and SSM weights from their original GPU addresses; the narrower `DIRECT_L0_NORM_WEIGHTS`, `DIRECT_L0_QKV_WEIGHTS`, `DIRECT_L0_PROJ_WEIGHTS`, and `DIRECT_L0_SSM_WEIGHTS` flags enable that behavior per substage. When direct weights cover the requested L0 stages and no explicit contract scaffold is requested, the runtime skips the duplicate device weightpack allocation/copy for that L0 replacement dispatch. The L0 projection replacement applies any post-matmul `z`/`beta`/`alpha` scale tensor before writing the public graph tensors, while preserving the unscaled raw value for the underlying `MUL_MAT` tensor when it exists. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_TRACE=1` enables the verbose per-evaluation diagnostics; keep it off for timing. Graph capture is disabled for L0 replacement evaluations while this path is being validated. The artifact still records the full 40-layer pack layout, but the runtime device pack defaults to the implemented L0 tensor subset to avoid duplicating the full model in VRAM. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_FULL_WEIGHTPACK=1` restores the full duplicate device pack and is expected to exceed 24 GB on the target model. The dispatch path launches one compiled cooperative physical contract kernel that calls 40 static layer blocks, reads through `device_pack + layer_descs`, reads runtime bindings, and optionally runs L0 math row-parallel in-block. It is still mostly scaffold, but it is a single physical dispatch over the packed model data, not a ggml layer loop.

The weightpack files define the persistent layout and the runtime log reports the process-local device buffer pointer as `device_pack=...`. The next implementation step is replacing the first recurrent GDN state update with direct reads from the SSM/L2/projection scratch outputs and packed weights.
