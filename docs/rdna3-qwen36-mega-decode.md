...
`GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_SMOKE=1` is kept as an alias for `DISPATCH=1`. `DISPATCH=1` alone no longer materializes or launches per-token work; it only enables the channel. `SUPERLAYER=1` is the explicit enable, but `REQUIRED`, `CONTRACT`, `RUN_L0`, `SINGLE_L0_DISPATCH`, or any `REPLACE_L0*` flag also implies the physical superlayer path. Set `CONTRACT=1` to materialize the runtime pack and launch the scaffold cooperative contract kernel without L0 math. Set `RUN_L0=1` to execute the L0 RMSNorm/QKV/projection math without skipping the normal graph. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_REPLACE_L0` also defaults to `0`; setting it to `1` implies L0 math and skips all replaced L0 graph nodes. The narrower `REPLACE_L0_RMS`, `REPLACE_L0_QKV`, and `REPLACE_L0_PROJ` flags replace only that L0 substage and are intended for correctness isolation. The still narrower `REPLACE_L0_PROJ_Z`, `REPLACE_L0_PROJ_BETA`, and `REPLACE_L0_PROJ_ALPHA` flags replace only one projection branch and skip only that branch's graph nodes. `SINGLE_L0_DISPATCH=1` is a full L0 physical preset: it implies RMS, QKV, Z, BETA, and ALPHA replacement and collapses them into one cooperative dispatch at `norm-0` with stage mask `0x1f`. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_FINAL=1` bypasses the L0 scaffold and enables the full Qwen3.6 mega-decode contract instead: it implies `GGML_CUDA_RDNA3_QWEN36_MEGA_DECODE=1`, `GGML_CUDA_RDNA3_QWEN36_FASTPATH=1`, linear MMVQ fastpath, required mode, graph-required mode, backend sampling, token-only sampler output, raw-logits readback suppression, async input copies, and reasoning disabled by default unless the user explicitly passes `--reasoning on`. Final mode currently uses a backend greedy sampler for token-only validation; re-enable stochastic top-k/temperature only after that backend sampler chain is proven fully offloadable on the target ROCm build. L0/one-layer physical replacement remains opt-in under final mode via `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_FINAL_PHYSICAL_L0=1`, so correctness can be validated before replacing graph math. Its explicit diagnostic prefix is `rdna3_qwen36_final`; final mode reports the first ready decode and true decode failures, while per-evaluation diagnostics require `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_TRACE=1`, so `GGML_CUDA_RDNA3_GRAPH_LOG=1` does not print once per generated token. Partial projection isolation still uses the latest enabled projection endpoint unless `SINGLE_L0_DISPATCH=1` is set. `REPLACE_L0_PROJ_Z_MATH_ONLY=1` computes and skips only the raw `z` matmul, then lets the graph run the public `z` node for isolation. `DIRECT_L0_PROJ_WEIGHTS=1` keeps the same physical kernel dispatch but reads L0 projection tensors from their original GPU addresses instead of the duplicate runtime weightpack. The L0 projection replacement applies any post-matmul `z`/`beta`/`alpha` scale tensor before writing the public graph tensors, while preserving the unscaled raw value for the underlying `MUL_MAT` tensor when it exists. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_TRACE=1` enables the verbose per-evaluation diagnostics; keep it off for timing. Graph capture is disabled for L0 replacement evaluations while this path is being validated. The artifact still records the full 40-layer pack layout, but the runtime device pack defaults to the implemented L0 tensor subset to avoid duplicating the full model in VRAM. `GGML_CUDA_RDNA3_QWEN36_SUPERLAYER_FULL_WEIGHTPACK=1` restores the full duplicate device pack and is expected to exceed 24 GB on the target model. The dispatch path launches one compiled cooperative physical contract kernel that calls 40 static layer blocks, reads through `device_pack + layer_descs`, reads runtime bindings, and optionally runs L0 math row-parallel in-block. It is still mostly scaffold, but it is a single physical dispatch over the packed model data, not a ggml layer loop.
